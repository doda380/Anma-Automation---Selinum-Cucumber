...
public class BookingSteps {
    ...
    public void select_provider_and_slot() {
        ...
        bookingPage.selectNumberOfAvailableSlots(5);
    }
    ...
    public void payment_method(String paymentMethod) {
    ...
    // PRICE ASSERTIONS
    @Then("The price calculations should be correct")
    public void verify_price_calculations() {

        int total = Integer.parseInt(bookingPage.getTotalAmount());
        int discount = Integer.parseInt(bookingPage.getDiscountAmount());
        int wallet = Integer.parseInt(bookingPage.getDeductedWalletAmount());
        int finalTotal = Integer.parseInt(bookingPage.getFinalTotal());

        int expected;

        if (bookingPage.isWalletEnabled()) {
            expected = total - discount - wallet;
        } else {
            expected = total - discount;
        }

        Assert.assertEquals(finalTotal, expected, "Wrong calculation detected!");
    }
